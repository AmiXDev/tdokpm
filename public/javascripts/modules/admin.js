...
socket.on('chat message', function(data) {
	...
	if (data.isAdmin)
		var $usernameDiv = $('<span class="username"/>').text("پشتیبان");
	else
		var $usernameDiv = $('<span class="username"/>').text("مشتری");
...
socket.on('typing', function(data) {
	...
	if (data.isTyping)
		$typing.append("<small>" + data.person + " در حال نوشتن میباشد.....<small>");
	else
		$typing.text('');
})
...
function getChatArea(id) {
	return ("<div class='chatArea'><div class='chatHeader'></div><ul class='messages'>" +
		"</ul><div class='typing'></div><input class='inputMessage' id='" + id + "'' placeholder='پیام خود را اینجا وارد کنید.'/></div>");
}
